...
playData=read.csv('NFL Play by Play 2009-2018 (v5).csv')
...
YearVec=seq(2009,2018)
firstDayVec=c("2009-09-10","2010-09-09","2011-09-08","2012-09-05","2013-09-05",
              "2014-09-04","2015-09-10","2016-09-08","2017-09-07","2018-09-06")

lastDayVec=c("2010-02-07","2011-02-06","2012-02-05","2013-02-03","2014-02-02",
...
#Teams Run to pass ratio (for current season)
  #season/teams lists
SeasonDataList=list(Data2009,Data2010,Data2011,Data2012,Data2013,Data2014,Data2015,Data2016,Data2017,Data2018)
...
NumericalData=playData[selectVec]
NumericalData=na.omit(NumericalData)

#split dataset
set.seed(4444)
sample=sample(seq_len(nrow(NumericalData)),size=floor(nrow(NumericalData)*.75))
train=NumericalData[sample,]
test=NumericalData[-sample,]
...
normdf <- as.data.frame(lapply(NumericalData[,1:26], normalize))
normdf=cbind(normdf,typeDummyVar=NumericalData$typeDummyVar)
normtrain=na.omit(normdf[sample,])
normtest=na.omit(normdf[-sample,])

#Determine Vars with colinearity issues:
  #Removals decided with log regression importance
...
#NumericalData<-NumericalData[!unlist(lapply(NumericalData, `%in%`, colinVars))]
